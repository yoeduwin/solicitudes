...
console.log('1. instalar() y directorio'); instalar();
assert(listarUsuarios_(false).length===5,'directorio con 5 personas');
guardarUsuario_({nombre:'Jimmy Ayala',correo:'jimmy@ea.mx',area:'Atención a Clientes',id:listarUsuarios_(true).filter(u=>u.nombre==='Jimmy Ayala')[0].id,admin:false});
guardarUsuario_({nombre:'Eduwin',correo:'eduwin@ea.mx',area:'Dirección',id:listarUsuarios_(true).filter(u=>u.nombre==='Eduwin')[0].id,admin:true});
...
assert(eduwin.admin===true,'permiso de Administración conservado');
try{guardarUsuario_({nombre:'Duplicado',correo:'JIMMY@EA.MX',area:'Prueba',activo:true});throw new Error('x');}
...
const r2=crearSolicitud_({solicitante_id:jimmy.id,responsable_id:eduwin.id,categoria:'Otro',
  prioridad:'Normal',titulo:'Revisar contrato',descripcion:'x',fecha_limite:hoyISO_()},eduwin);
...
const c1=agregarComentario_(r2.solicitud.id,eduwin,{comentario:'Sin prisa',notificar:false});
assert(c1.correo_enviado===false,'sin notificar => sin correo');
MAILS.length=0;
const c2=agregarComentario_(r2.solicitud.id,eduwin,{comentario:'Ya urge',notificar:true});
assert(c2.correo_enviado===true && MAILS[0].to==='jimmy@ea.mx','notificar => correo a la contraparte');
const det=detalleSolicitud_(r2.solicitud.id);
assert(det.comentarios.length===2,'2 comentarios');
assert(det.comentarios.every(c=>c.autor_nombre==='Eduwin'),'comentarios firmados por la sesión');
...
assert(ini.datos.sesion.usuario && ini.datos.sesion.usuario.nombre==='Eduwin','sesión resuelta por correo');
